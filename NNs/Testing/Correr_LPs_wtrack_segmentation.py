...
video  = "20231030_113309.mp4"
cap = cv2.VideoCapture(video_path + video)
output_dir = "ids/"

def check_str(box, seg_mask, frame):
    ...
    result_strings = []
    X1, Y1, X2, Y2 = box
    car_img = result[int(Y1):int(Y2), int(X1):int(X2)]
...
while cap.isOpened():
    ...
    if success:
        ...
        results = model.track(frame, persist=True, classes = [2, 3, 5, 7], verbose = False, max_det = 6)
        ...
        if results[0].boxes.id is not None:
            ...
            for box, seg_mask, track_id in zip(boxes, seg_masks, track_ids):
                strings = check_str(box, seg_mask, frame)
